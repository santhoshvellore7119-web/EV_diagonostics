...
Accurate, non-invasive detection of early-stage lithium-ion cell degradation mechanisms (specifically lithium plating, gas formation, and active material dissolution) relies on capturing subtle shifts in ultrasonic acoustic velocity ($) and acoustic attenuation ($lpha$). 

In a standard prismatic or pouch lithium-ion battery cell with a nominal thickness  = 10\text{ mm}$, ultrasonic longitudinal waves propagate with a baseline speed of sound  \approx 2200 - 2600\text{ m/s}$. The round-trip pulse-echo acoustic baseline Time-of-Flight (ToF) is:
...
| **Clock Stability (TCXO)** | \text{ MHz, } \pm 0.5\text{ ppm}$ | \text{ MHz, } \pm 0.5\text{ ppm}$ | Internal RC Oscillator ($\pm 1\%$) |
...
### 3.1 Subsystem 1: 4-Cell Diagnostic Sensing Front-End (Target: < .00)
| Item # | Component | Manufacturer & Part Number | Description / Specs | Unit Cost (1k Qty) | Total Subsystem Cost |
|---|---|---|---|---|---|
| 1.1 | PZT Transducers (x4 pair) | Audiowell / Piezo Hannas | 2.5 MHz PZT-5H disc, 10mm dia | .85 / ea (x8 = .80) | .80 |
| 1.2 | Ultrasonic Pulser IC | Microchip / TI MD1210 + TC6320 | Dual high-speed ultrasound pulser | .40 | .40 |
| 1.3 | Analog Front End / LNA | Analog Devices AD8065ARZ | 145 MHz FET input ultra-low noise op-amp | .85 | .85 |
| 1.4 | High-Speed Comparator | Texas Instruments TLV3501 | 4.5 ns Rail-to-Rail high-speed comparator | .20 | .20 |
| 1.5 | Time-to-Digital Converter | Texas Instruments TDC7200PWR | 55 ps resolution picosecond stopwatch | .85 | .85 |
| 1.6 | Multi-Channel Multiplexer | Analog Devices ADG704BRUZ | 4-channel low-capacitance RF switch | .40 | .40 |
| 1.7 | Main Microcontroller | Espressif ESP32-S3-WROOM-1 | Dual-core 240MHz, 8MB Flash, 2MB PSRAM | .75 | .75 |
| 1.8 | Precision Current/Voltage Sensor | Texas Instruments INA226AIDGSR | 16-bit ^2C$ Bi-directional Current/Power Monitor | .65 | .65 |
| 1.9 | NTC Thermistor Array (x4) | TDK NTCG103UH103HT1 | 10k $\Omega$, 0.5% tolerance, 0.05C resolution | .20 / ea (x4 = .80) | .80 |
| 1.10 | Power Management & LDOs | TI TPS7A4700 / AP7361C | Low-noise RF LDO (3.3V / 5.0V / +12V boost) | .80 | .80 |
| 1.11 | PCB, Passives, Connectors | 4-Layer FR4 Impedance Controlled | PCB fabrication, discrete passives, SMA/JST | .50 | .50 |
| **TOTAL SENSING BOM** | | | | | **.00** |
...
| Item # | Component | Manufacturer & Part Number | Description / Specs | Unit Cost (1k Qty) | Total Subsystem Cost |
|---|---|---|---|---|---|
| 2.1 | Synchronous Buck-Boost Controller | TI LM5176PWPR | Bi-directional 4-switch Buck-Boost Controller | .60 | .60 |
| 2.2 | Dual Power MOSFETs (x2) | Alpha & Omega AON6500 | 30V 40A, {DS(on)} < 2.5\text{ m}\Omega$ | .65 / ea (x4 = .60) | .60 |
| 2.3 | Power Inductor | Wurth Elektronik 7443321000 | 10 $\mu\text{H}$, 15A Saturation, High Q | .40 | .40 |
| 2.4 | Galvanic Digital Isolator | Texas Instruments ISO7741 | 5 kV RMS Quad-Channel Digital Isolator | .95 | .95 |
| 2.5 | Hardware Safety Disconnect Relay | Panasonic AQY282EH Solid-State | Phototriac / PhotoMOS optical fail-safe relay | .20 | .20 |
| 2.6 | Shunt Resistor & Gate Drivers | Vishay WSL2512 + TI UCC27211 | \text{ m}\Omega$ 1% shunt + high-speed gate driver | .10 | .10 |
| 2.7 | Thermal Dissipation & Connectors | Custom Aluminum Extrusion | Thermal pad, chassis sink, automotive terminals | .50 | .50 |
| **TOTAL REBALANCING BOM** | | | | | **.35** |

---

### 3.3 Complete Production System Cost
- **Total Diagnostic Sensing BOM (4-Cell Module)**: **.00** (Well within <  target)
- **Total Active Power Rebalancing BOM**: **.35**
- **Combined Integrated System**: **.35** (at 1,000 unit volume)
...
3. **Fail-Safe Default**: In the absence of software heartbeat or in brownout conditions, the hardware pull-down resistors force the power stage into high-impedance open-circuit isolation ( = 0.00\text{ A}$).
