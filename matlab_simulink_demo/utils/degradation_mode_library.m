...
%   See also load_parameters, estimate_ecm_params_rls
    ...
    base_modes('healthy') = struct( ...
        'R0_scale', 1.000, 'R1_scale', 1.000, 'C1_scale', 1.000, ...
        'sos_factor', 1.000, 'attenuation_factor', 1.000, 'phase_factor', 0.000, ...
        'R_th_scale', 1.000, 'C_th_scale', 1.000, 'heat_factor', 1.000);

    % Lithium plating (recoverable phase boundary)
    base_modes('li_plating') = struct( ...
        'R0_scale', 1.156, 'R1_scale', 1.120, 'C1_scale', 0.933, ...
        'sos_factor', 1.024, 'attenuation_factor', 0.960, 'phase_factor', 3.14159, ...
        'R_th_scale', 1.050, 'C_th_scale', 0.990, 'heat_factor', 1.050);

    % Severe lithium plating (dendritic growth)
    base_modes('severe_li_plating') = struct( ...
        'R0_scale', 1.350, 'R1_scale', 1.250, 'C1_scale', 0.850, ...
        'sos_factor', 1.035, 'attenuation_factor', 0.920, 'phase_factor', 3.14159, ...
        'R_th_scale', 1.120, 'C_th_scale', 0.950, 'heat_factor', 1.150);

    % Active material loss (loss of cyclable lithium & cathode dissolution)
    base_modes('active_material_loss') = struct( ...
        'R0_scale', 1.333, 'R1_scale', 1.400, 'C1_scale', 0.733, ...
        'sos_factor', 0.960, 'attenuation_factor', 0.920, 'phase_factor', 0.000, ...
        'R_th_scale', 1.100, 'C_th_scale', 0.950, 'heat_factor', 1.100);

    % Electrolyte decomposition (SEI layer growth & solvent depletion)
    base_modes('electrolyte_decomposition') = struct( ...
        'R0_scale', 1.956, 'R1_scale', 2.200, 'C1_scale', 0.567, ...
        'sos_factor', 0.952, 'attenuation_factor', 0.780, 'phase_factor', 0.200, ...
        'R_th_scale', 1.200, 'C_th_scale', 0.920, 'heat_factor', 1.250);

    % Gas generation (pouch swelling & acoustic reverberation)
    base_modes('gas_generation') = struct( ...
        'R0_scale', 1.222, 'R1_scale', 1.200, 'C1_scale', 0.867, ...
        'sos_factor', 0.840, 'attenuation_factor', 0.600, 'phase_factor', 0.500, ...
        'R_th_scale', 1.350, 'C_th_scale', 0.880, 'heat_factor', 1.300);

    % Internal micro-short (separator puncture & localized Joule heating)
    base_modes('internal_short') = struct( ...
        'R0_scale', 2.111, 'R1_scale', 2.600, 'C1_scale', 0.467, ...
        'sos_factor', 0.968, 'attenuation_factor', 0.750, 'phase_factor', 0.000, ...
        'R_th_scale', 1.500, 'C_th_scale', 0.850, 'heat_factor', 2.500);
