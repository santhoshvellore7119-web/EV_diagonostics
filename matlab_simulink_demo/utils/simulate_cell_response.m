...
%       response - struct containing electrical, ultrasonic, and thermal responses
    ...
    % Improved thermal simulation using exact discretization
    % Thermal system: dT_rise/dt = -T_rise/(R_th*C_th) + power_in/C_th
    % where power_in = energy_per_sample * heat_factor
    % This has the same form as the RC circuit: dy/dt = -y/tau + x/tau*R
    % Exact discretization: y[k] = y[k-1]*exp(-dt/tau) + x[k]*(1-exp(-dt/tau))*R

    thermal_time_constant = R_th * C_th;
    thermal_alpha = exp(-dt / thermal_time_constant);
    thermal_beta = R_th * (1 - thermal_alpha);  % This gives the steady-state gain

    % Calculate power_in for each time step
    power_in = energy_per_sample * heat_factor;

    for i = 2:length(t)
        temperature_rise(i) = temperature_rise(i-1) * thermal_alpha + power_in(i-1) * (1 - thermal_alpha);
    end

    % Calculate final dT/dt
    if length(t) > 1
        dT_dt_final = (power_in(end) - temperature_rise(end) / R_th) / C_th;
    else
        dT_dt_final = 0;
    end
